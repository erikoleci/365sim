...
const FeaturedMatchCard: React.FC<FeaturedMatchCardProps> = ({ match, onBetClick, onOpenDetail, selectedIds }) => {
  ...
  const getButtonClass = (marketId: string, selectionId: string) => {
    ...
    const base = 'flex-1 flex flex-col items-center justify-center py-1.5 rounded text-[11px] font-bold transition-colors';
  ...
  return (
    <div className="shrink-0 w-52 bg-brand-panel border border-brand-divider rounded overflow-hidden hover:border-brand-yellow/50 transition-colors">
      <button onClick={() => onOpenDetail(match)} className="w-full text-left px-2.5 pt-2 pb-2">
        <div className="flex items-center justify-between text-[10px] text-brand-textMuted mb-1.5">
          ...
          <span className="opacity-60">{shortId}</span>
        </div>
      ...
      {matchWinnerMarket && (
        <div className="flex gap-1 px-2 pb-2">
          {matchWinnerMarket.options.map((opt) =>
            opt.suspended ? (
              <div key={opt.id} className="flex-1 flex items-center justify-center py-1.5 rounded text-[11px] opacity-40" title="Tregu është pezulluar përkohësisht">🔒</div>
            ) : (
              <button
                key={opt.id}
